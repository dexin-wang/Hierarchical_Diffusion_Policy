...
def extract(a, t, x_shape):
    ...
    """
    b, *_ = t.shape
    out = a.gather(-1, t)   # shape=(B,) out[i] = a[..., t[i]]
...
def cosine_beta_schedule(timesteps, s = 0.008):
    ...
    """
    steps = timesteps + 1
    x = np.linspace(0, steps, steps)    # 0~timesteps
    alphas_cumprod = np.cos(((x / steps) + s) / (1 + s) * np.pi * 0.5) ** 2 # IDDPM alpha累乘, 共timesteps+1个
    alphas_cumprod = alphas_cumprod / alphas_cumprod[0]
    betas = 1 - (alphas_cumprod[1:] / alphas_cumprod[:-1])  # 计算beta，去除里x=0处的alpha/beta，保留1~timesteps的beta
