...
from hiera_diffusion_policy.common.robot import get_subgoals_stage_real, get_subgoals_stage_moveT
from hiera_diffusion_policy.common.visual import visual_subgoals_real_v6, create_tbox_pts
...
class RealReplayDataset(BasePcdDataset):
    def __init__(self,
        ...
        with h5py.File(dataset_path) as file:
            ...
            tbox_pcd_700 = create_tbox_pts()
            tbox_pcd_2100 = create_tbox_pts(num=2100)

            if mode == 'push':
                demo_range = range(192)[:128]
            elif mode == 'grasp':
                demo_range = range(192)[128:]
            else:
                demo_range = range(192)

            # 遍历轨迹，获取 state / action
            for i in tqdm(demo_range, desc="Loading hdf5 to ReplayBuffer"):
                ...
                if use_subgoal:
                    ...
                    subgoals = get_subgoals_stage_moveT(
                        demo['obs']['state'],
                        tbox_pcd_2100,
                        fin_rad=0.015,
                        sim_thresh=[0.002, 1./180*np.pi],
                        Tr=Tr,
...
def _data_to_obs(raw_obs, tbox_pcd, raw_actions, abs_action, rotation_transformer, Tr):
    """
    args:
    ...
    # 构造点云
    tbox_pcd_batch = np.expand_dims(tbox_pcd, axis=0).repeat(state.shape[0], axis=0)
    tbox_pcd_state = tf.transPts_tq_npbatch(tbox_pcd_batch, state[:, -7:-4], state[:, -4:])

    data = {
        'pcd': tbox_pcd_state[:-Tr],
        'state': state[:-Tr],
        'action': raw_actions[:-Tr],

        'next_pcd': tbox_pcd_state[Tr:],
        'next_state': state[Tr:],
