...
from hiera_diffusion_policy.model.common.module_attr_mixin import ModuleAttrMixin
...
class LowdimMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        ...
        obs_mask = obs_mask & is_obs_dim    # 同时满足 horizon < self.max_n_obs_steps 和 dim 为obs
...
class PcdMaskGenerator(ModuleAttrMixin):
    def __init__(self,
        action_dim,
    ...
    def forward(self, shape):
        ...
        # generate dim mask
        dim_mask = torch.zeros(size=shape, dtype=torch.bool, device=device)
        dim_mask[:, :(self.observation_history_num-1), :] = True
