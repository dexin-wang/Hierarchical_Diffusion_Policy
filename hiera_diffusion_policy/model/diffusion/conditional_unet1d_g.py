...
from hiera_diffusion_policy.model.diffusion.positional_embedding import SinusoidalPosEmb
...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self, 
        ...
        """
        sample = einops.rearrange(sample, 'b h t -> b t h') # (batch, dim, horizon)，其中, dim=channel
        ...
        timesteps = timesteps.expand(sample.shape[0])   # 无作用

        global_feature = self.diffusion_step_encoder(timesteps) # sin-cos position emb (batch, 256)

        if self.use_subgoal and subgoal is not None: # None
            global_feature = torch.cat([global_feature, subgoal], axis=-1)
        ...
        for idx, (resnet, resnet2, downsample) in enumerate(self.down_modules):
            ...
            # if idx == 0 and len(h_local) > 0:   # not use
            #     x = x + h_local[0]
        ...
        x = einops.rearrange(x, 'b t h -> b h t')   # (batch, horizon, in_channel) 和输入sample的shape一样
        return x
