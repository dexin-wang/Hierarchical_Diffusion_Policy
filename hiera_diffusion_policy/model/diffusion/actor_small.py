...
from hiera_diffusion_policy.model.diffusion.pointnet_pointnet2 import PointNet2Encoder
...
class Actor(nn.Module):
    def __init__(
        ...
        if use_pcd:
            if isinstance(self.pcd_encoder, PointNet2Encoder):
                cond_dim += self.pcd_encoder.out_dim*2
            else:
    ...
    def forward(
        ...
        if self.use_pcd and pcd is not None:
            if isinstance(self.pcd_encoder, PointNet2Encoder):
                pcd_num = pcd.shape[-1]//3
                pcd_emb = list()
                for i in range(pcd_num):
                    pcd_emb.append(self.pcd_encoder(pcd[..., 3*i:3*(i+1)]))
                pcd_emb = torch.concat(tuple(pcd_emb), -1)

            else:
                pcd_emb = self.pcd_encoder(pcd)
            cond += (pcd_emb,)
        
        # print('3D encoder time:', time.time()-t1)
