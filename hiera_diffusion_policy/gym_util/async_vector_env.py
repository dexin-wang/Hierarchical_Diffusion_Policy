...
class AsyncVectorEnv(VectorEnv):
    ...
    def goal_async(self):
        ...
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
                "Calling `goal_async` while waiting "
                "for a pending call to `{0}` to complete".format(self._state.value),
                self._state.value,
            )
        ...
        for pipe in self.parent_pipes:
            pipe.send(("goal", None))
        self._state = AsyncState.WAITING_GOAL

    def goal_wait(self, timeout=None):
        """
        Parameters
        ----------
        timeout : int or float, optional
        ...
        """
        self._assert_is_running()
        if self._state != AsyncState.WAITING_GOAL:
            raise NoAsyncCallError(
                "Calling `goal_wait` without any prior " "call to `goal_async`.",
                AsyncState.WAITING_GOAL.value,
            )

        if not self._poll(timeout):
            self._state = AsyncState.DEFAULT
            raise mp.TimeoutError(
                "The call to `goal_wait` has timed out after "
                "{0} second{1}.".format(timeout, "s" if timeout > 1 else "")
            )
        ...
        self._raise_if_errors(successes)
        self._state = AsyncState.DEFAULT
        scene_pcds, object_pcds, goals = zip(*results)
        
        return (
            np.array(scene_pcds),
            np.array(object_pcds),
            np.array(goals),
        ) 
    ...
    def render_img(self, *args, **kwargs):
        return self.call('render_img', *args, **kwargs)
    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                scene_pcd, object_pcd, goal = env.goal()
                pipe.send(((scene_pcd, object_pcd, goal), True))
            elif command == "step":
                ...
                if name in ["goal", "reset", "step", "seed", "close"]:
                    raise ValueError(
                ...
                raise RuntimeError(
                    ...
                    "be one of {'goal', `reset`, `step`, `seed`, `close`, "
