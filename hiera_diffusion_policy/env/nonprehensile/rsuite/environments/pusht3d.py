...
from hiera_diffusion_policy.env.nonprehensile.rsuite.models.objects.xml_objects import TBoxObject
...
操作物体：T型刚体
action维度：xyz平移
完成条件：相比于目标位姿，物体的xy平移差小于2mm，z轴旋转差小于2°
"""

class PushT3D(SingleArmEnv):
    """
    ...
    def __init__(
        self,
        robots,
        coord_x_range=[-0.1, 0.1],
        coord_y_range=[-0.1, 0.1],
        rota_axis='z', # 'x','y','z'
        rota_range=[-3.14, 3.14],
        friction_range=[0.95, 0.95],
        density_range=[50, 50],
        table_friction_range=[0.5, 0.5],
        goal_offset_th=0.003,
        goal_rotate_th=3./180*np.pi,
        ...
        self.goal_offset_th = goal_offset_th
        self.goal_rotate_th = goal_rotate_th

        #* 设置目标位姿
        self.goal_pose = np.array([0, 0, 0.82, 0, 0, 0.3826834, 0.9238795]) # pos+qua(xyzw)
    ...
    def _load_model(self):
        ...
        self.obj = TBoxObject(name='tbox')
        self.stl_path = self.obj.stl_path
        self.obj_target = TBoxObject(name="tbox_target", visual=True)
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.obj, self.obj_target],
        )
    ...
    def remove_all_objects(self):
        ...
        #* 放置目标物体
        cube_target_id = self.sim.model.body_name2id('tbox_target_main')
        self.sim.model.body_pos[cube_target_id] = np.array([6, 0, 5])
        self.sim.model.body_quat[cube_target_id] = np.array([1, 0, 0, 0])
    ...
    def _reset_internal(self):
        ...
        #* 放置目标物体
        cube_target_id = self.sim.model.body_name2id('tbox_target_main')
        self.sim.model.body_pos[cube_target_id] = self.goal_pose[:3]
        self.sim.model.body_quat[cube_target_id] = np.array([
            self.goal_pose[6], self.goal_pose[3], self.goal_pose[4], self.goal_pose[5]])
    ...
    def _check_success(self):
        ...
        obj_qua = convert_quat(np.array(self.sim.data.body_xquat[self.obj_body_id]), to="xyzw")
        obj_euler = tf.Qua_to_Euler(obj_qua)
        goal_rz = tf.Qua_to_Euler(self.goal_pose[3:])

        offset = max(abs(obj_pos[0]-self.goal_pose[0]), abs(obj_pos[1]-self.goal_pose[1]))
        rotate = tf.angle_diff(obj_euler[2], goal_rz[2])

        if offset < self.goal_offset_th and rotate < self.goal_rotate_th:
            return True
        else:
            return False
