...
from hiera_diffusion_policy.env.pusht.pusht_env import PushTEnv
from hiera_diffusion_policy.env.pusht.pymunk_keypoint_manager import PymunkKeypointManager
import numpy as np
...
class PushTKeypointsEnv(PushTEnv):
    ...
    def _get_obs(self):
        ...
        if not self.agent_keypoints:    # <--
            ...
            # ])
            #! 修改：关键点+agent位置+T物体pose
            obj_pose = np.array(tuple(self.block.position) + (self.block.angle,))
            obs = np.concatenate([obs, agent_pos, obj_pose])
            obs_mask = np.concatenate([
